...
class Solution:
    def checkOverlap(self, radius: int, xCenter: int, yCenter: int, x1: int, y1: int, x2: int, y2: int) -> bool:
        def distance2circle(x, y):
            return pow((x - xCenter) ** 2 + (y - yCenter) ** 2, 0.5)
        if x1 <= xCenter <= x2 and y1 <= yCenter <= y2:
            ...
            return distance2circle(x1, y1) <= radius or distance2circle(x1, y2) <= radius or distance2circle(x2, y1) <= radius or distance2circle(x2, y2) <= radius
